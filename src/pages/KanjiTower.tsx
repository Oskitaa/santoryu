...
export default function KanjiTower({ embedded = false }: { embedded?: boolean }) {
  ...
  const filteredKanji = useMemo(() => {
    ...
    if (categoryFilter === 'numbers') return kanjiList.slice(0, 14);
    if (categoryFilter === 'nature') return kanjiList.slice(14, 21);
    if (categoryFilter === 'people') return kanjiList.slice(21, 29);
    if (categoryFilter === 'actions') return kanjiList.slice(29);
    return kanjiList;
  ...
  const content = (
    <div className="space-y-5 animate-fade-in px-1 max-w-lg mx-auto pb-6">
      ...
      {/* Detail Modal with High Z-Index & Bottom Clearance */}
      <AnimatePresence>
        {selectedKanji && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-md p-0 sm:p-4 pb-[calc(env(safe-area-inset-bottom,0px)+3.75rem)] sm:pb-4"
          ...
          >
            <motion.div
              ...
              animate={{ y: 0 }}
              exit={{ y: 100 }}
              className="w-full max-w-lg bg-[var(--color-bg-surface)] border border-white/10 rounded-t-3xl sm:rounded-3xl p-6 space-y-4 max-h-[78vh] overflow-y-auto shadow-2xl pb-8"
